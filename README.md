...
| onMenuItemClick | `(value, extras) => window.location.href = '#' + value +'&token=' + extras.token` | This property enables you to provide custom onClick method for items. The function gets passed one parameter which is the value of the clicked item. If you do not provide an onMenuItemClick. [Demo(without extras)](react-sidemenu.demo.com/#custom-click )|
